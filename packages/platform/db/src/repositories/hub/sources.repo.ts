/**
 * InstitutionalSource Repository — registered ingestion sources.
 */
...
export function makeSourceRepo(db: HubPrismaClient) {
  return {
    findBySourceId: (sourceId: string) =>
      db.institutionalSource.findUnique({ where: { sourceId } }),

    list: () =>
      db.institutionalSource.findMany({
