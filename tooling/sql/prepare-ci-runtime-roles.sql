...
BEGIN
  FOREACH app_name IN ARRAY ARRAY['core', 'hub', 'spot', 'seumei', 'contracts', 'willdash', 'ops', 'pay'] LOOP
  ...
  FOREACH app_name IN ARRAY ARRAY['core', 'hub', 'spot', 'seumei', 'contracts', 'willdash', 'ops', 'pay'] LOOP
    ...
    END IF;
    EXECUTE format('GRANT CREATE ON DATABASE %I TO %I', current_database(), 'matriz_' || app_name || '_migration');
