...
interface Target { app: string; config: string; identity: string; workflow: string; tag: string }
const targets: readonly Target[] = [
  ...
  { app: "matriz-uninstall", config: "apps/matriz-uninstall/desktop/tauri/src-tauri/tauri.conf.json", identity: "com.matriz.uninstall.tauri", workflow: ".github/workflows/matriz-uninstall-windows.yml", tag: "uninstall-v*" },
  { app: "matriz-uninstall", config: "apps/matriz-uninstall/package.json", identity: "com.matriz.uninstall.electron", workflow: ".github/workflows/matriz-uninstall-electron-windows-release.yml", tag: "uninstall-electron-v*" },
  { app: "matriz-workbench", config: "apps/matriz-workbench/electron-builder.config.cjs", identity: "com.matriz.workbench", workflow: ".github/workflows/workbench-windows-release.yml", tag: "workbench-v*" },
  { app: "seumeiapp", config: "apps/seumeiapp/desktop/electron-builder.yml", identity: "com.matriz.seumei", workflow: ".github/workflows/seumei-windows-release.yml", tag: "seumei-v*" },
]
...
export function verifyDesktopReleaseMatrix(root: string): string[] {
  ...
  for (const target of targets) {
    ...
    const packageVersion = packageText ? JSON.parse(packageText).version : ""
    if (packageVersion && !/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(packageVersion)) errors.push(`${target.app}: package version must be valid semver`)
    if (manifestText && packageVersion && !new RegExp(`version:\\s*["']${escape(packageVersion)}["']`).test(manifestText)) errors.push(`${target.app}: manifest version must match package version ${packageVersion}`)
    if (configText && target.config.endsWith("tauri.conf.json") && packageVersion && JSON.parse(configText).version !== packageVersion) errors.push(`${target.app}: Tauri version must match package version ${packageVersion}`)
