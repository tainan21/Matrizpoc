...
export function presentProducts(
  catalog: readonly DistributionProductV1[],
  ...
  return catalog.map((product) => {
    ...
    const newer = Boolean(
      observed?.version &&
        ...
        compareSemver(product.release.version, observed.version) > 0,
    )
    const status: ProductStatus = observed
      ? observed.publisher === product.windows.publisher
        ? newer
          ? "outdated"
          : "installed"
        : "inconsistent"
      : product.state === "active" && product.release
        ? "available"
        : "unavailable"
    const actions: ProductAction[] =
      status === "installed" || status === "outdated"
        ? [
            ...(status === "outdated" ? ["update" as const] : []),
            "reinstall",
            "uninstall",
            "cleanup",
          ]
...
function reconcileInstallations(
  ...
  const edges = catalog
    .flatMap((product) =>
      installed.map((observation) => ({
        product,
        observation,
        score: matchScore(product, observation),
      })),
    )
  ...
  const result = new Map<string, InstalledProduct>()
  for (const edge of edges) {
    if (products.has(edge.product.productId) || installations.has(edge.observation.installationId))
      continue
    products.add(edge.product.productId)
    installations.add(edge.observation.installationId)
    result.set(edge.product.productId, edge.observation)
  }
  return result
}
...
function matchScore(product: DistributionProductV1, installed: InstalledProduct) {
  ...
  return product.windows.aliases.some((name) => lower(name) === lower(installed.displayName))
    ? 1
    : 0
}
...
const labels: Record<ProductStatus, string> = {
  ...
  unavailable: "Sem instalador publicado",
  inconsistent: "Instalação inconsistente",
}
