...
export async function createReleaseAction(form: FormData) {
  const productId = text(form, "productId")
  await createDistributionGatewayFromEnvironment().createRelease(
    productId,
    {
      ...
      installer: {
        ...
        downloadUrl: text(form, "downloadUrl"),
...
export async function changeProductStateAction(form: FormData) {
  const productId = text(form, "productId")
  await createDistributionGatewayFromEnvironment().updateProduct(
    productId,
    { state: text(form, "state") as "active" | "unavailable" | "retired" },
    `admin:state:${productId}:${randomUUID()}`,
  )
  revalidatePath("/distribution")
}
...
export async function publishReleaseAction(form: FormData) {
  ...
  await createDistributionGatewayFromEnvironment().publishRelease(
    releaseId,
    `admin:publish:${releaseId}:${randomUUID()}`,
  )
  revalidatePath("/distribution")
}
...
export async function retireReleaseAction(form: FormData) {
  const releaseId = text(form, "releaseId")
  await createDistributionGatewayFromEnvironment().retireRelease(
    releaseId,
    `admin:retire:${releaseId}:${randomUUID()}`,
  )
  revalidatePath("/distribution")
}
