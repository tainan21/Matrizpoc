...
if (path.dirname(installationPath) !== outputRoot || path.dirname(lifecyclePath) !== outputRoot) throw new Error("Installed evidence must belong to the acceptance run")
...
for (const id of ["INST-001", "INST-002", "INST-003", "INST-005"]) passedIds.add(id)
const evidenceFile = `${runId}/playwright-evidence.json`
const directInstallerEvidence = new Set(["INST-001", "INST-002", "INST-003", "INST-005"])
