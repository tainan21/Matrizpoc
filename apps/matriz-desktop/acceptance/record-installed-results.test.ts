...
it("certifies only contract cases backed by passed mapped journeys", async () => {
  ...
  try {
    ...
    expect(results.filter((entry) => entry.status === "pass").map((entry) => entry.id)).toEqual(["TERM-001", "TERM-002", "TERM-003", "TERM-007", "INST-001", "INST-002", "INST-003", "INST-005", "INST-006"])
    ...
    expect(JSON.parse(await readFile(join(output, "summary.json"), "utf8"))).toMatchObject({ status: "blocked", passed: 9, blocked: results.length - 9 })
