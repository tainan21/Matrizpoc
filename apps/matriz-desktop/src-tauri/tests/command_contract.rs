...
fn native_inventory_has_one_entry_for_every_renderer_command() {
    assert_eq!(COMMAND_NAMES.len(), 91);
...
fn native_inventory_keeps_sensitive_operations_explicit() {
    ...
    assert!(COMMAND_NAMES.contains(&"terminal_readiness"));
    ...
    assert!(COMMAND_NAMES.contains(&"activate_package"));
    assert!(COMMAND_NAMES.contains(&"recover_runtime"));
    assert!(COMMAND_NAMES.contains(&"run_runbook"));
    assert!(COMMAND_NAMES.contains(&"stage_git_changes"));
    assert!(COMMAND_NAMES.contains(&"unstage_git_changes"));
    assert!(COMMAND_NAMES.contains(&"commit_git_changes"));
    assert!(COMMAND_NAMES.contains(&"run_git_remote"));
    assert!(COMMAND_NAMES.contains(&"check_update"));
    assert!(COMMAND_NAMES.contains(&"download_update"));
    assert!(COMMAND_NAMES.contains(&"install_update"));
    assert!(COMMAND_NAMES.contains(&"preview_store_install"));
    assert!(COMMAND_NAMES.contains(&"confirm_store_install"));
}
