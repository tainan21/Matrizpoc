...
function PackageCard({ item, busy, selected, inspect, open }: { item: StorePackage; busy: boolean; selected: boolean; inspect(): void; open(): void }) {
  return <article className={`package-card${selected ? " is-selected" : ""}`}>
    ...
    <div className="package-state">{item.builtIn ? <Badge tone="success">BUILT-IN</Badge> : item.installed ? <Badge tone="success">INSTALADO</Badge> : <Badge tone="neutral">INDISPONÍVEL</Badge>}</div>
    <div className="package-actions" onClick={(event) => event.stopPropagation()}>
      {item.installed ? <Button disabled={busy} aria-label={`Abrir ${item.name}`} onClick={open}>ABRIR</Button> : null}
    </div>
  </article>
}
