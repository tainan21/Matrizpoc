...
export function createTauriGateway(
  ...
  return {
    ...
    gitBranch: (request) => invoke(commands.gitBranch, { request }),
    previewGitMerge: (revision, target) => invoke(commands.previewGitMerge, { revision, target }),
    confirmGitMerge: (confirmationToken) => invoke(commands.confirmGitMerge, { confirmationToken }),
    abortGitMerge: (revision) => invoke(commands.abortGitMerge, { revision }),
    ...
    infrastructureEventDiagnostics: () => invoke(commands.infrastructureEventDiagnostics),
    ...
    previewStoreInstall: (packageId) => invoke(commands.previewStoreInstall, { packageId }),
    confirmStoreInstall: (confirmationToken) => invoke(commands.confirmStoreInstall, { confirmationToken }),
