...
export function App() {
  ...
  useEffect(() => {
    window.naevia.downloads().then(setDownloads).catch((cause) => setError(String(cause)))
    return window.naevia.subscribeDownloads(setDownloads)
  }, [])
...
function formatBytes(value: number) { return value < 1_000_000 ? `${Math.round(value / 1_000)} KB` : `${(value / 1_000_000).toFixed(1)} MB` }
