...
export async function POST(request: Request, { params }: Ctx) { try { const { id } = await params; const actor = await getDocsActorContextFromRequest(request); const suggestion = await makeDocsRepository().reviewSuggestion(actor, id, "rejected"); if (wantsHtmlRedirect(request)) return docsRedirect(request, "/docs/suggestions"); return NextResponse.json({ ok: true, suggestion }) } catch (error) { return docsErrorResponse(error) } }
