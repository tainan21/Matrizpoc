...
const deploymentOriginVariables = [
  "MATRIZ_SPOT_ORIGIN",
  "MATRIZ_SEUMEI_ORIGIN",
  "MATRIZ_CONTRACTS_ORIGIN",
  "MATRIZ_WILLDASH_ORIGIN",
  "MATRIZ_WORKBENCH_ORIGIN",
  "MATRIZ_SITES_ORIGIN",
] as const
...
function trustedOrigin(value: string, allowWebSocket = false, production = false): string {
  ...
  const validProtocol = origin.protocol === "https:" || (!production && isLocalHttp) ||
    (!production && allowWebSocket && (origin.protocol === "ws:" || origin.protocol === "wss:"))
  if (!validProtocol || origin.username || origin.password || origin.pathname !== "/" || origin.search || origin.hash) {
    throw new Error("Invalid trusted connect origin.")
  }
  return origin.origin
}

/** Server-only deployment configuration; localhost is deliberately development-only. */
...
export function buildContentSecurityPolicy(
  ...
  const connectSources = [
    "'self'",
    ...(options.connectOrigins ?? []).map((origin) => trustedOrigin(origin, false, options.production)),
    ...(options.developmentWebSocketOrigin ? [trustedOrigin(options.developmentWebSocketOrigin, true, options.production)] : []),
  ]
