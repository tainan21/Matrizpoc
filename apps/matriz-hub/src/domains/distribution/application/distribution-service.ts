...
import { distributionProductInputV1Schema, distributionReleaseInputV1Schema } from "@matriz/integration-api-contracts"
...
export class DistributionService {
  constructor(private readonly repository: DistributionRepository, private readonly now = () => new Date()) {}
  createProduct(actor: DistributionActor, input: DistributionProductInputV1, idempotencyKey: string) {
  ...
  private authorize(actor: DistributionActor) {
    if (!actor.capabilities.includes("distribution.catalog.manage")) throw new Error("Missing capability distribution.catalog.manage")
  }
}

function requiredKey(value: string) {
  if (!/^[A-Za-z0-9._:-]{8,160}$/.test(value)) throw new Error("A valid idempotency key is required")
  return value
}
