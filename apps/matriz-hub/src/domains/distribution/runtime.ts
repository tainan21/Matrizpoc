...
const globalRuntime = globalThis as typeof globalThis & { __matrizDistributionService?: DistributionService }
const repository = process.env.HUB_DATABASE_URL ? createPrismaDistributionRepository() : createMemoryDistributionRepository()
export const distributionService = globalRuntime.__matrizDistributionService ??= new DistributionService(repository)
...
function serviceActor(request: Request) {
  ...
  if (expected.length < 16 || provided.length !== expected.length || !timingSafeEqual(Buffer.from(provided), Buffer.from(expected))) return null
  return { userId: "service:matriz-admin", capabilities: ["distribution.catalog.manage"] }
}

export const distributionHttp = createDistributionHttpHandlers(distributionService, { authorize: serviceActor, ready })
