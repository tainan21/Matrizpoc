...
export function InfrastructureCockpit({ inventory }: { inventory: InfrastructureInventoryViewModel }) {
  ...
  async function requestEnvironmentExport(appId: string) {
    if (!bridge) return
    try { setEnvironmentPreview(await bridge.invoke({ type: "infrastructure.local.environment.preview", appId }) as LocalEnvironmentExportPreview); setMessage(null) }
    catch (error) { setMessage(error instanceof Error ? error.message : "Exportação de ambiente recusada") }
  }
  async function confirmEnvironmentExport() {
    if (!bridge || !environmentPreview) return
    try {
      const result = await bridge.invoke({ type: "infrastructure.local.environment.confirm", confirmationToken: environmentPreview.confirmationToken }) as { state: "exported"; appId: string }
      setEnvironmentPreview(null)
      setMessage(`Ambiente local de ${result.appId} exportado com ACL restrita.`)
    }
