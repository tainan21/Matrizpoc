...
export function AppsConsole() {
  ...
  useEffect(() => { void fetch("/api/projects").then((response) => response.json()).then((data: { projects: TerminalProject[] }) => { setProjects(data.projects) }).catch(() => setError("Não foi possível ler os projetos.")) }, [])
