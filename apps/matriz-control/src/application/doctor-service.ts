...
function contained(parent: string, child: string) { const rel = relative(parent, child); return rel !== ".." && !rel.startsWith(`..\\`) && !rel.startsWith("../") && !isAbsolute(rel) }
...
export class DoctorService {
  ...
  async previewCleanup(projectId: string, targetId: string) {
    ...
    const path = resolve(project.path, folders[targetId as keyof typeof folders]); if (!contained(project.path, path)) throw new Error("Cleanup path outside project")
  ...
  async confirmCleanup(token: string) {
    ...
    const expected = resolve(project.path, folders[preview.targetId]); if (expected !== preview.path || !contained(project.path, expected)) throw new Error("Cleanup target changed")
