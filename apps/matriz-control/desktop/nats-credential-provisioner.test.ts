...
describe("WindowsNatsCredentialProvisioner", () => {
  it("retrieves the DPAPI-protected password without putting it in arguments and returns only its bcrypt verifier", async () => {
    const execute = vi.fn(async () => JSON.stringify({ payPassword: "p".repeat(64), controlPassword: "c".repeat(64) }))
    ...
    expect(prepared).toMatchObject({ payPasswordHash: "$2b$12$" + "x".repeat(53), controlPasswordHash: "$2b$12$" + "x".repeat(53) })
    expect(execute).toHaveBeenCalledWith("powershell.exe", ["-NoProfile", "-NonInteractive", "-ExecutionPolicy", "Bypass", "-File", "C:/Matriz/nats-credential-helper.ps1", "-Action", "ProvisionPay"])
