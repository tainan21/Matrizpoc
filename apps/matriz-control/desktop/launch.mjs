...
async function waitForControl() {
  for (let attempt = 0; attempt < 80; attempt += 1) {
    try { const response = await fetch("http://127.0.0.1:3009/unlock", { redirect: "manual" }); if (response.status < 500) return }
    catch { /* server is still starting */ }
    await new Promise((resolve) => setTimeout(resolve, 250))
  }
  throw new Error("Matriz Control web runtime did not start on port 3009")
}
