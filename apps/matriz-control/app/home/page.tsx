...
export default async function HomePage() {
  ...
  const [git, projects] = await Promise.allSettled([new GitCliRepository(root).overview().then(presentGitOverview), listTerminalProjects(root).then((items) => items.map(({ id, name, port }) => ({ id, name, port })))])
  const input: HomeInput = {
    ...
    doctor: { status: "rejected", reason: "Carregue o Doctor para atualizar o diagnóstico." },
  }
