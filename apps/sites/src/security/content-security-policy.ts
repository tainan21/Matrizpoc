export interface ContentSecurityPolicyOptions { development?: boolean; production?: boolean; connectOrigins?: readonly string[]; developmentWebSocketOrigin?: string }
type OriginEnvironment = Readonly<Record<string, string | undefined>>
const developmentOrigins = ["http://localhost:3000"] as const
const deploymentOriginVariables = ["MATRIZ_HUB_ORIGIN"] as const
function trustedOrigin(value: string, allowWebSocket = false, production = false): string {
  ...
  if (!(origin.protocol === "https:" || (!production && isLocalHttp) || (!production && allowWebSocket && (origin.protocol === "ws:" || origin.protocol === "wss:"))) || origin.username || origin.password || origin.pathname !== "/" || origin.search || origin.hash) throw new Error("Invalid trusted connect origin.")
