  ...
  return (
    <aside className="seumei-context-sidebar" data-expanded={expanded}>
      ...
      <nav aria-label="Aplicação atual" data-expanded={String(expanded)}>
        ...
        {navigation.map((item, index) => <Link key={item.id} href={item.href} className={index === 0 && activeApp ? "is-active" : ""}><span aria-hidden="true">{shellIcon(item.icon ?? (index === 0 ? activeApp?.icon ?? "spark" : "spark"))}</span><span>{item.label}</span></Link>)}
      </nav>
