...
export async function readStoreDesignHandler(actor: SessionActor, companyId: string, services: StoreDesignHttpServices): Promise<HttpResult> { try { return { status: 200, body: { draft: await readStoreDesign(await context(actor, companyId, services), services.storeDesign) } } } catch (error) { return errorResult(error) } }
...
export async function saveStoreDesignHandler(actor: SessionActor, companyId: string, body: unknown, services: StoreDesignHttpServices): Promise<HttpResult> {
  ...
  try { return { status: 200, body: { draft: await saveStoreDesign(await context(actor, companyId, services), body as never, services.storeDesign) } } } catch (error) { return errorResult(error) }
}
