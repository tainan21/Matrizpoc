...
export default async function ProductsPage() {
  ...
  try { const context = await resolveActiveCompanyContext(foundation.actor, foundation.preferredCompanyId, foundation.services.core, foundation.services.companies); const catalog = toCatalogViewModel(await readCatalog(context, foundation.services.catalog))
    return <main className="catalog-page"><header><div><span className="eyebrow">OPERAÇÃO / CATÁLOGO</span><h1>Produtos</h1><p>{catalog.products.length} produtos · {catalog.categories.length} categorias</p></div>{catalog.canManage && <Link className="catalog-primary" href="/workspace/products/new">Novo produto</Link>}</header>
